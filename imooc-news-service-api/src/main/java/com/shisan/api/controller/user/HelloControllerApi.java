...
public interface HelloControllerApi {
     ...
     *    老板不会去问员工，他只会对接部门经理；
     *    那么这里的所有的api 接口就是统一在这里进行管理和调度的，微服务也是如此；
     * @return
     */
     ...
     * 耦合度高了，接口的目的为了能够提供解耦
     * <p>
     * 此外，本工程的接口就是一套规范，实现都是在各自的工程去做的处理。
     * 目前我们使用 springboot 作为接口实现的。
     * 如果未来以后出现新的 java web框架，那么我们不需要修改接口，
     * 只需要去修改对应的实现就可以了。这其实也是一个解耦的体现。
     * <p>
     * Swagger2，基于接口的自动文档生成。
     * 所有的配置文件只需要一份，就能在当前项目中去构建了。
     * 管理起来很方便
     * <p>
     * 综上所述，如此做法，可以提高多服务的项目可扩展性。
     *
     * @return
     */
